...
###Fast Req Methods:
    ...
    def RequestWingPlaneDirection(self):
        ...
        self.LERT = np.matmul(self.wing_rotation_relative_to_inertia, np.matmul(self.virturalWingPlaneRelative2Wing.T,\
                                                                             Normalized_LeadingEdgeRoot2Tip_in_wing_frame))
        ## Note that LERT is resolved in the inertia frame. BLEwing(virtualWing) -> 
        Normalized_chordDirecLeading2Trailing        = np.matrix([[0], [1], [0]])
        self.chordD = np.matmul(self.wing_rotation_relative_to_inertia,np.matmul(self.virturalWingPlaneRelative2Wing.T,\
...
### Main implementations:
    def CopmputeAerodynamicForce(self):
        ...
        Sec_Norm_eFV = self.SecNorm(self.eFV  )  ##added_mass_FV
        ...
        ### All the subsequent forces are resolved in the inertia frame.
        ...
        # please note that the following are resolved in the BLE file frame
        ...
        #these raw_pos are resolved in the wing frame
        raw_pos_t    =   np.array(np.matmul(self.virturalWingPlaneRelative2Wing.T, np.array([raw_X_pos_t, raw_Y_pos_t, 0]))).squeeze()
        raw_pos_r    =   np.array(np.matmul(self.virturalWingPlaneRelative2Wing.T, np.array([raw_X_pos_r, raw_Y_pos_r, 0]))).squeeze()
        raw_pos_a    =   np.array(np.matmul(self.virturalWingPlaneRelative2Wing.T, np.array([raw_X_pos_a, raw_Y_pos_a, 0]))).squeeze()
        
        self. raw_X_pos_t = raw_X_pos_t
        self. raw_Y_pos_t = raw_Y_pos_t
        # print(raw_pos_t)
        self. X_pos_t = raw_pos_t[0]
        self. Y_pos_t = raw_pos_t[1]
        self. Z_pos_t = raw_pos_t[2]
        
        self. X_pos_r = raw_pos_r[0]
        self. Y_pos_r = raw_pos_r[1]
        self. Z_pos_r = raw_pos_r[2]
        
        self. X_pos_a = raw_pos_a[0]
        self. Y_pos_a = raw_pos_a[1]
        self. Z_pos_a = raw_pos_a[2]
